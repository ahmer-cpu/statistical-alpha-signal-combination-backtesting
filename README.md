...
A Python research project for **building, validating, and combining cross-sectional
equity signals**, with a backtesting and validation layer designed to separate signal
from noise.

Every factor is gated through Newey-West-corrected IC tests, block-bootstrap confidence
intervals, IC-decay curves, and a turnover/cost-aware backtest before it enters a combined
signal.
...
and market regimes, the research settles on a single combined signal:

> **Signed IC-weighted (EWM, 126-day half-life) blend of sector-neutral momentum + low-volatility,
> rebalanced every 31 days.**
>
> - **Annualized return (in-sample):** 13.53%
> - **Market-neutral:** β ≈ −0.02 to SPY
> - **Full-sample Sharpe ≈ 0.99** (cumulative PnL +73%, hit rate 56%) at 10 bps rebalancing cost
> - **Per-regime split:** Sharpe +0.89 through the 2022 bear (a positive return while SPY fell ~18%)
>   and +1.03 through the bull

These are in-sample figures on a single bear and a single bull, so they describe this sample
rather than an out-of-sample expectation. The largest contribution to drawdown occurs at regime
changes and in high-volatility periods. The results do not claim tradeable alpha; the main caveats:

- Membership/selection bias: the study used current S&P 100 members' history over the sample
  period, not point-in-time membership. This biases the results upward.
- Structural lookahead: statistical validation ran over the full sample before factor selection.
  This could be avoided with true train/test splits, though the factor selection here was unchanged.
- A much larger study over more assets and history is needed to ascertain the results (the period
  contains only one extended bear market).

Naive IC significance tests on overlapping forward returns overstate confidence. Applying a
Newey-West HAC correction to the IC t-statistics deflated apparent significance by ~3.5×, enough
to turn several "significant" single factors into null results. The Lasso combiner (rolling Lasso
regression on forward returns, using the coefficients as factor weights) gave similar results.

---

## What's inside

- **Factor library:** an abstract `Alpha` base class with **13 factor implementations**
...
- **Signal validation:** autocorrelation, turnover, cross-sectional dispersion, lead-lag profiles,
  sub-period stability, leave-one-out, and sector splits, orchestrated by a one-call `SignalReport`
...
| [04: Alpha Research & Combination](notebooks/04_alpha_research.ipynb) | 13-factor triage, IC-correlation redundancy map, three-combiner comparison, regime/transition stress tests, and the chosen combined signal |
